...
/* --- Version 5.03 --- */
var UI_VERSION = '5.03';
var ppp_firmware_enabled = false;
var portal_address = 'http://www.wowweesupport.com/portal';
var hostIP="http://192.168.0.11/";
var pureHostIP="192.168.0.11";
...
function getPlayerCode(width, height){ 
    var path = pureHostIP; // hostIP; //location.hostname;
    var httppath = path;
   ...
   // make sure quicktime is installed, if not default to mjpeg
    ...
    switch(parseInt(v_player)){
        case 0:
         ...
         case 1:
            return '<object id="oCamCtl" CLASSID="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" width="100%" height="100%" CODEBASE="qtplugin.cab">' +
                    ...
                    '<embed src="cam.mp4" scale="ToFit" wmode="transparent" volume="255" qtsrc="rtsp://' + rtsp_udp_path + '/webcam" width="100%" height="100%" autoplay="true" loop="false" controller="false" type="video/quicktime" pluginspage="download"></embed>' +
                    '</object>';
/*            return '<object id="oCamCtl" CLASSID="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" width="' + width + '" height="' + height + '" CODEBASE="http://www.apple.com/qtactivex/qtplugin.cab">' +
                    ...
                    '<param name="scale" value="ToFit">' +
                    '<param name="volume" value="255">' +
                    '<embed src="cam.mp4" scale="ToFit" wmode="transparent" volume="255" qtsrc="rtsp://' + rtsp_udp_path + '/webcam" width="' + width + '" height="' + height + '" autoplay="true" loop="false" controller="false" pluginspage="http://www.apple.com/quicktime/"></embed>' +
                    '</object>';
					*/
            break;
...
// alert('<img id="oCamCtl" src="http://' + httppath + '/GetData.cgi?' + stream_id + '" width="100%" height="100%" />');
            return ('<img id="oCamCtl" src="http://' + httppath + '/GetData.cgi?' + stream_id + '" width="100%" height="100%" />'); 
            break;
    }
/*
            v_player = 2;
            closeStream();
            stream_id = Math.floor (10000 * Math.random ());
            Player.setVideoURL("http://" + httppath + "/GetData.cgi?" + stream_id);
			return "";
            return ('<img id="oCamCtl" src="http://' + httppath + '/GetData.cgi?' + stream_id + '" width="' + width + 'px" height="' + height + 'px" />'); 
			v_player = 1;
			//closeStream();
			// Player.setVideoURL("rtsp://" + rtsp_udp_path + "/webcam/cam.mp4");
			Player.setVideoURL("http://" + httppath + "/GetData.cgi");
			Player.playVideo();
return "";
            v_player = 0;
            closeStream();
            ...
            // return ('<video id="oCamCtl" src="http://' + httppath + '/GetData.cgi?' + stream_id + '" width="' + width + 'px" height="' + height + 'px" />'); 
            return ('<video id="oCamCtl" src="rtsp://' + rtsp_tcp_path + '/webcam" width="' + width + 'px" height="' + height + 'px" />'); 

            v_player = 1;
			Player.setVideoURL("rtsp://" + rtsp_tcp_path + "/webcam");
			Player.playVideo();


v_player = 4;
//return '<img id="jpgimg" src="'+hostIP+'Jpeg/CamImg' + Math.floor (10000 * Math.random ()) + '.jpg" alt="Loading Snapshot..." />';
return hostIP+'Jpeg/CamImg' + Math.floor (10000 * Math.random ()) + '.jpg';
*/
}
...
function continuousPhoto()
{
	showPhoto();
	setTimeout('continuousPhoto();',500);
}
function showPhoto(){
	kodas = hostIP+'Jpeg/CamImg' + Math.floor (10000 * Math.random ()) + '.jpg'
	if ($("jpgimg") == undefined)
	{
		// $('camera_container').innerHTML = '<img id="jpgimg" src="'+hostIP+'Jpeg/CamImg' + Math.floor (10000 * Math.random ()) + '.jpg" alt="Loading Snapshot..." />';
        $('camera_container').setStyle({background: 'url("'+kodas+'") top left'});
	}
	else
	{
		// $('jpgimg').src=hostIP+'Jpeg/CamImg' + Math.floor (10000 * Math.random ()) + '.jpg';
        $('camera_container').setStyle({background: 'url("'+kodas+'") top left'});
	}
}
